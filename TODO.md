...
| Sekce                              | Stav        | Poznámka                                                                                                                        |
| ---------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Datový model (ER)                  | 🟢 Ready    | [docs/data-model.md](docs/data-model.md) — ~70 entit, pole, enumy, FK. Nejsilnější část.                                        |
...
| Bankovní synchronizace (Fio)       | 🟢 Ready    | [docs/fio-sync.md](docs/fio-sync.md) — token, okno stahování, idempotence, souběh a rate limit, chybové stavy.                                   |
...
| Role a oprávnění                   | 🟢 Ready    | [docs/authorization.md](docs/authorization.md) — pět vrstev, matice po doménách, citlivá data.                                  |
| Reporty                            | 🟢 Ready    | [docs/reports.md](docs/reports.md) — metriky, parametry, scope, hrany.                                      |
...
| UI / obrazovky / toky              | 🟡 Partial  | `docs/du-doc-ux-pruvodce.md` — inventář flow a specifikace obrazovek. Zbývá uzavřít rozhodnutí, navigační vrstva, texty, vizuál. |
...
- Část rozhodnutí byla vlastně **změna spec, ne UX**. Všech pět je hotových a zapsaných v `docs/`: odmítnutí přihlášky zákonným zástupcem ([registration-lifecycle.md](docs/registration-lifecycle.md)), token pro `RECOMMENDATION` a chybějící cena pro typ účastníka ([validation.md](docs/validation.md)), placená akce bez bankovního účtu (tamtéž → **Akce**) a znovuposlání ztraceného odkazu ([non-functional.md](docs/non-functional.md) → **Tokeny a ochrana přístupu**). Zbytek `[K rozhodnutí]` v průvodci je skutečně UX.
