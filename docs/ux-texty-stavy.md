...
Stav se **nikdy nezobrazuje jako jediné sdělení** — hlavní je checklist „co ještě chybí" (§ 2), název stavu je jen doplňkový štítek (M3 assist/filter chip, tonální, ikona vlevo). Jedna sdílená komponenta pro tokenový rozcestník (`/stav/:token`), self-management (`/muj-ucet/prihlasky/:pid`) i detail v oddílové správě. Výjimkou je náhradník ve stavu `New`: ten aktivní checklist nemá, protože dokumenty a platbu řeší až po přijetí nabídky místa.
...
| Úprava použité ceny              | Řádek ceníku, na který ukazuje `price_id` alespoň jedné přihlášky, nelze upravit ani smazat; tlačítka jsou disabled.                                                                               | „Tuto cenu už použila alespoň jedna přihláška, nelze ji upravit ani smazat. Založte novou platnost.“ |
| Úprava použité položky číselníku | Položku s existující volbou (`REGISTRATION_FIELD_VALUE`) nelze upravit ani smazat; tlačítka jsou disabled.                                                                                         | „Tuto položku už někdo zvolil, nelze ji upravit ani smazat. Založte novou položku.“                  |
...
| Docházka                         | Přítomen / Pozdě / Nepřítomen / Omluven předem jsou čtyři různé stavy, nezapsáno je jejich absence; „Nepřítomen“ vyžaduje důvod, ostatní stavy ho nesmí mít; nejvýše jeden záznam na osobu a akci. | „Vyberte důvod nepřítomnosti.“ / jinak bez validační chyby, změna se ukládá okamžitě.                |
...
| Přeplatek                        | Nabídnout Vrátit / Převést na jinou přihlášku téže osoby.                                                                                                                                          | „Vyberte, jak naložit s přeplatkem X Kč.“                                                            |
...
| Změna hesla — Nové heslo        | `ACCOUNT`                                     | min. 12 znaků, zadává se dvakrát — [validation.md](validation.md#formáty). „Heslo musí mít alespoň 12 znaků.“ / „Hesla se neshodují.“                                        |
