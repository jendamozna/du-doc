...
- Revert provádí **administrátor**, jen ze stavu `completed`, a jen dokud existuje `MERGE_LOG`. Snapshot spadá do **3leté retence auditního logu** (README → **Retence a GDPR**, řádek „Auditní logy (merge…)“); po jejím uplynutí se `MERGE_LOG` smaže a **sloučení už nelze vrátit** — stav zůstane `completed` natrvalo.
